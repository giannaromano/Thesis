...
rmarkdown::render(
  "StatusReports.Rmd",
  output_format = rmarkdown::html_document(self_contained = TRUE),  # inline all images/libs
  output_file  = "index.html",                                      # Pages looks for index.html
  output_dir   = "docs",
  envir        = new.env()                                          # clean run; no stale objects
)
```
...
```{r LoadData}
HeartAttack <- read.csv("~/Documents/Thesis/HeartAttack.csv")
```

Status Report #1:
Clean data, find outliers, create data dictionary.
```{r StatusReport1}
# Load packages
library(dplyr)

# Head data to see variable names/see noticiable missing values
head(HeartAttack)     # Varible names look appropriate and no missing values seen yet
